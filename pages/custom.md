...
![Image 8: Edit Text Formatting](../../assets/edittextformattingcustompages.png) 
...
![Image 8: Edit Text Formatting](../../assets/textcolor.png)
To change the text color on a custom page, begin by selecting the word, sentence, or paragraph that requires modification; if no text is selected, the color change will apply to any new text typed afterward. Next, select the `Text Color` icon in the Rich Text Editor menu. A color selector will then appear, offering multiple ways to select a desired color. Choose the desired color from the preset options or use a custom color selector if available. Users may select a base hue using the horizontal rainbow slider, which determines the color family displayed in the main selection panel above. Within the large gradient field, colors can be adjusted by clicking or dragging to control saturation (left to right) and brightness (top to bottom). An eyedropper tool allows sampling a color directly from anywhere on the screen, automatically updating the selector to match the sampled value. For exact color matching, users can input RGB, HSL, or HEX values in the field at the bottom of the color selector. A preview swatch displays the currently selected color, providing immediate visual confirmation of any adjustments. Once a color is selected, the selected text will update immediately. 

![Image 8: Edit Text Formatting](../../assets/highlightcolor.png)
To apply highlighting to text, select the relevant text and click the `Highlight` icon from the Rich Text Editor menu. The color selector will then appear. Choose the preferred highlight color; this applies a background fill behind the selected text. If the result needs adjustment, reselect the text and either choose a different color or use the `Undo` button.
